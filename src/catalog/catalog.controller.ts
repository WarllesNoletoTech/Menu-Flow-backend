...
@Controller('restaurants/:restaurantId/manage-catalog') @Roles(Role.SUPER_ADMIN) @UseGuards(JwtGuard, RolesGuard)
export class AdminCatalogController { constructor(private readonly catalog: CatalogService) {} @Get('categories') categories(@Param('restaurantId') id:string){return this.catalog.manageCategories(id)} @Get('products') products(@Param('restaurantId') id:string){return this.catalog.manageProducts(id)} @Post('categories') createCategory(@Param('restaurantId') id:string,@Body() body:CreateCategoryDto){return this.catalog.createCategory(id,body)} @Patch('categories/reorder') reorderCategories(@Param('restaurantId') id:string,@Body() body:ReorderItemDto[]){return this.catalog.reorderCategories(id,body)} @Patch('categories/:categoryId') updateCategory(@Param('restaurantId') id:string,@Param('categoryId') categoryId:string,@Body() body:Partial<CreateCategoryDto>){return this.catalog.updateCategory(id,categoryId,body)} @Post('products') createProduct(@Param('restaurantId') id:string,@Body() body:CreateProductDto){return this.catalog.createProduct(id,body)} @Patch('products/reorder') reorderProducts(@Param('restaurantId') id:string,@Body() body:ReorderItemDto[]){return this.catalog.reorderProducts(id,body)} @Patch('products/:productId') updateProduct(@Param('restaurantId') id:string,@Param('productId') productId:string,@Body() body:Partial<CreateProductDto>){return this.catalog.updateProduct(id,productId,body)} }
