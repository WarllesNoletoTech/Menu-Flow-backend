...
type ProductUpdateInput = Partial<Omit<Product, 'categoryId' | 'restaurantId'>> & { categoryId?: string };
@Injectable() export class CatalogService {
  ...
  async updateProduct(restaurantId: string, id: string, input: ProductUpdateInput) { if (input.categoryId && !(await this.categories.exists({ _id: input.categoryId, restaurantId }))) throw new NotFoundException('Category not found'); const product = await this.products.findOneAndUpdate({ _id: id, restaurantId }, input, { new: true, runValidators: true }); if (!product) throw new NotFoundException('Product not found'); return product; }
}
