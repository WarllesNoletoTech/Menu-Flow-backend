...
export type CheckoutItem = {
  productId: string;
  quantity: number;
  addons?: Array<{ groupId: string; addonId: string }>;
  addonNames?: string[];
  observation?: string;
};
...
export function expectedChangeCents(
  paymentMethod: string,
  needsChange: boolean | undefined,
  changeForCents: number | undefined,
  totalCents: number,
) {
  if (paymentMethod !== "CASH") {
    if (needsChange || changeForCents !== undefined)
      throw new BadRequestException(
        "Troco só está disponível para pagamento em dinheiro.",
      );
    return undefined;
  }
  if (!needsChange) return undefined;
  if (!Number.isInteger(changeForCents) || changeForCents! < totalCents)
    throw new BadRequestException(
...
export function deliveryAddressSnapshot(
  address: Record<string, string>,
  restaurant: { city?: string; state?: string },
) {
  const clean = (field: string, max: number) =>
    String(address[field] ?? "")
      ...
      .slice(0, max);
  return {
    zipCode: clean("zipCode", 10),
    street: clean("street", 160),
    number: clean("number", 30),
    neighborhood: clean("neighborhood", 100),
    complement: clean("complement", 160),
...
export function zoneMatchesNeighborhood(
  zone: { coverageType?: string; name: string },
  neighborhood: string,
) {
  return (
    zone.coverageType === DeliveryCoverageType.ALL ||
    (zone.coverageType !== DeliveryCoverageType.ALL &&
      zone.name.localeCompare(neighborhood, "pt-BR", {
        sensitivity: "base",
      }) === 0)
  );
}
...
export function deliveryFeeForZone(
  ...
  return zoneMatchesNeighborhood(zone, neighborhood)
    ? cents(zone.feeCents, zone.fee)
    : undefined;
}

export function addressBelongsToRestaurant(
  address: Record<string, string>,
  restaurant: { city?: string; state?: string },
) {
  ...
  return (
    (!city ||
      city.localeCompare(restaurant.city ?? "", "pt-BR", {
        sensitivity: "base",
      }) === 0) &&
...
export class OrdersService {
  constructor(
    ...
    private readonly settings: Model<RestaurantSettings>,
  ...
  async create(
    ...
    const authenticatedCustomerId = customerId
      ? this.objectId(customerId, "Cliente autenticado inválido.")
      : undefined;
    if (idempotencyKey) {
      ...
      if (existing) {
        ...
        return this.checkoutResponse(existing, restaurant!);
      }
    }
    const [restaurant, settings] = await Promise.all([
      this.restaurants.findById(rid).lean(),
      this.settings.findOne({ restaurantId: rid }).lean(),
    ]);
    if (!restaurant || restaurant.blocked)
      throw new NotFoundException("Estabelecimento não encontrado.");
    ...
    if (!availability.canAcceptOrdersNow)
      throw new BadRequestException(
        "Este estabelecimento não está recebendo pedidos agora.",
      );
    if (input.fulfillment === "PICKUP" && settings?.pickupEnabled === false)
      throw new BadRequestException("Retirada no local não está disponível.");
    if (input.fulfillment === "DELIVERY" && !settings?.deliveryEnabled)
      throw new BadRequestException("Entrega não está disponível.");
    if (
      !(await this.payments.exists({
        restaurantId: rid,
        method: input.paymentMethod,
        active: true,
      }))
    )
      throw new BadRequestException("Forma de pagamento indisponível.");
    ...
    if (input.fulfillment === "DELIVERY") {
      ...
      if (!restaurant.city?.trim() || !restaurant.state?.trim())
        throw new BadRequestException(
          "A cidade e a UF do estabelecimento precisam estar configuradas para entrega.",
        );
      if (!addressBelongsToRestaurant(input.address!, restaurant)) {
        throw new BadRequestException(
          `Este estabelecimento realiza entregas somente em ${restaurant.city} - ${restaurant.state.toUpperCase()}.`,
        );
      }
    ...
    if (
      ...
      throw new BadRequestException(
        "Um produto pertence a uma categoria indisponível.",
      );
    ...
    const items = input.items.map((item) => {
      ...
      const selections =
        item.addons ?? this.legacySelections(product, item.addonNames ?? []);
      const unique = new Set(
        selections.map(
          (selection) => `${selection.groupId}:${selection.addonId}`,
        ),
      );
      if (unique.size !== selections.length)
        throw new BadRequestException(
          "Um adicional só pode ser selecionado uma vez.",
        );
      const selected = selections.map((selection) => {
        const group = product.addonGroups.find(
          (candidate: any) => candidate._id?.toString() === selection.groupId,
        );
        const addon = group?.addons.find(
          (candidate: any) => candidate._id?.toString() === selection.addonId,
        );
        if (!group || !addon)
          throw new BadRequestException(
            "Adicional inválido ou desatualizado. Atualize o cardápio.",
          );
      ...
      for (const group of product.addonGroups as any[]) {
        const count = selected.filter(
          (addon) => addon.groupId === group._id.toString(),
        ).length;
        const minimum = group.required
          ? Math.max(1, group.min ?? 1)
          : (group.min ?? 0);
        if (count < minimum || count > (group.max ?? 1))
          throw new BadRequestException(
    ...
    const subtotalCents = items.reduce(
      (sum, item) =>
        ...
        item.quantity *
          (item.unitPriceCents +
            item.addons.reduce((n, addon) => n + addon.priceCents, 0)),
      0,
    );
    const minimumOrderCents = cents(
      settings?.minimumOrderCents,
      settings?.minimumOrder,
    );
    if (subtotalCents < minimumOrderCents)
      throw new BadRequestException(
        `O pedido mínimo é ${(minimumOrderCents / 100).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}.`,
      );
    ...
    if (input.fulfillment === "DELIVERY") {
      ...
      const zone = input.deliveryZoneId
        ...
        : await this.zones
            .findOne({
              ...
              active: true,
              $or: [
                { coverageType: DeliveryCoverageType.ALL },
                {
                  coverageType: DeliveryCoverageType.SPECIFIC,
                  name: new RegExp(`^${escapeRegex(neighborhood)}$`, "i"),
                },
                {
                  coverageType: { $exists: false },
                  name: new RegExp(`^${escapeRegex(neighborhood)}$`, "i"),
                },
              ],
            })
      ...
      if (!zoneMatchesNeighborhood(zone, neighborhood))
        throw new BadRequestException(
          "A região selecionada não corresponde ao bairro informado.",
        );
    ...
    if (input.couponCode) {
      const coupon = await this.validCoupon(
        rid,
        input.couponCode,
        subtotalCents,
      );
    ...
    try {
      ...
      for (let attempt = 0; attempt < 5; attempt += 1) {
        try {
          order = await this.orders.create({
            ...
            needsChange,
            changeForCents: needsChange ? input.changeForCents : undefined,
            changeFor: needsChange ? input.changeForCents! / 100 : undefined,
            expectedChangeCents: calculatedChangeCents,
            items,
            ...
            statusHistory: [
              {
                ...
                ...(authenticatedCustomerId
                  ? { changedBy: authenticatedCustomerId }
                  : {}),
              },
            ],
          });
          ...
          if (
            (error as { code?: number; keyPattern?: Record<string, number> })
              .code !== 11000 ||
            (error as any).keyPattern?.orderNumber !== 1 ||
            attempt === 4
          )
            throw error;
        }
      ...
      await this.customers.findOneAndUpdate(
        { restaurantId: rid, phone: input.phone.trim() },
        {
          $set: { name: input.customerName.trim(), lastOrderAt: new Date() },
          ...(deliveryAddress
            ? { $addToSet: { addresses: deliveryAddress } }
            : {}),
          $inc: { orderCount: 1, totalSpent: totalCents / 100 },
        },
        { upsert: true },
      );
      ...
      if ((error as { code?: number }).code === 11000 && idempotencyKey) {
        const existing = await this.orders
          .findOne({ restaurantId: rid, idempotencyKey })
          .lean();
        if (existing) return this.checkoutResponse(existing, restaurant);
      }
      throw error;
    }
  }
  ...
  forCustomer(customerId: string) {
    ...
    return this.orders
      .find({ customerId: uid })
      .populate("restaurantId", "name tradeName slug address mapUrl")
      .sort({ createdAt: -1 })
      .limit(100)
      .lean();
  }
  async publicOrder(orderNumber: string, token: string) {
    const order = await this.orders
      .findOne({ orderNumber, publicToken: token })
      .populate("restaurantId", "name tradeName slug address mapUrl")
      .lean();
  ...
  async updateStatus(
    ...
    const actor = this.objectId(
      actorId,
      "Responsável pela alteração inválido.",
    );
    const order = await this.orders.findOne({ _id: oid, restaurantId: rid });
    if (!order) throw new NotFoundException("Pedido não encontrado.");
    ...
    try {
      ...
      if ((error as { name?: string }).name === "VersionError")
        throw new ConflictException("Este pedido já foi atualizado.");
      throw error;
    }
  ...
  async cancelByCustomer(customerId: string, id: string, reason?: string) {
    const order = await this.orders.findOne({ _id: id, customerId });
    if (!order) throw new NotFoundException("Pedido não encontrado.");
    if (order.status !== "PENDING")
      throw new ConflictException(
        "Após a aceitação, entre em contato com o estabelecimento.",
      );
  ...
  private validateAddress(address?: Record<string, string>) {
    for (const field of ["zipCode", "street", "number", "neighborhood"])
      if (typeof address?.[field] !== "string" || !address[field].trim())
        throw new BadRequestException(
          "Preencha o endereço completo para entrega.",
        );
    if (address!.neighborhood.trim().length > 100)
      throw new BadRequestException(
  ...
  private checkoutResponse(order: any, restaurant: Restaurant) {
    ...
    const whatsappMessage = buildOrderWhatsAppMessage(
      order,
      restaurant,
      `${frontend}${trackingUrl}`,
    );
    return {
      ...order,
      trackingUrl,
      whatsappUrl: buildWhatsAppUrl(restaurant.orderWhatsapp, whatsappMessage),
    };
  }
  private newOrderNumber() {
    const alphabet = "ABCDEFGHJKMNPQRSTUVWXYZ23456789";
    const bytes = randomBytes(6);
    return `MF-${Array.from(bytes, (byte) => alphabet[byte % alphabet.length]).join("")}`;
  }
  private objectId(value: string, message: string, notFound = false) {
    if (!Types.ObjectId.isValid(value)) {
      if (notFound) throw new NotFoundException(message);
  ...
  private legacySelections(product: any, names: string[]) {
    return names.map((name) => {
      const matches = product.addonGroups.flatMap((group: any) =>
        group.addons
          .filter((addon: any) => addon.name === name)
          .map((addon: any) => ({
            groupId: group._id?.toString(),
            addonId: addon._id?.toString(),
          })),
      );
      if (matches.length !== 1 || !matches[0].groupId || !matches[0].addonId)
        throw new BadRequestException(
          "Adicional antigo ambíguo. Selecione novamente.",
        );
      return matches[0];
    });
  }
  private async validCoupon(
    ...
    if (
      !coupon ||
      subtotalCents < Math.round(coupon.minimumOrder * 100) ||
  ...
  private async consumeCoupon(id: Types.ObjectId, orderId: Types.ObjectId) {
    const used = await this.coupons.updateOne(
      {
        _id: id,
        $or: [
    ...
    if (used.modifiedCount !== 1) {
      await this.orders.deleteOne({ _id: orderId });
      throw new BadRequestException("O limite do cupom foi atingido.");
    }
  }
}
