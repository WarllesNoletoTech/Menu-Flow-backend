...
import { normalizeReportWhatsapp } from './report-whatsapp';
...
class UpdateUserDto { @IsOptional() @IsString() @MinLength(1) name?: string; @IsOptional() @IsEmail() email?: string; @IsOptional() @IsString() phone?: string; @IsOptional() @IsString() reportWhatsapp?: string; }
...
export class UsersController {
  ...
  @Get() async list(@Query('page') requestedPage?: string, @Query('limit') requestedLimit?: string, @Query('search') search?: string, @Query('role') role?: Role, @Query('status') status = 'all') {
    ...
    const [items, total] = await Promise.all([this.users.find(filter).select('name email phone reportWhatsapp role active deletedAt restaurantId createdAt updatedAt').populate('restaurantId', 'name tradeName').sort({ createdAt: -1 }).skip((page - 1) * limit).limit(limit).lean(), this.users.countDocuments(filter)]);
  ...
  @Patch(':userId') async update(@Param('userId') id: string, @Body() body: UpdateUserDto) { const user = await this.find(id); const changes: Record<string, unknown> = {}; if (body.name !== undefined) changes.name = body.name.trim(); if (body.phone !== undefined) changes.phone = body.phone.trim(); if (body.reportWhatsapp !== undefined && user.role === Role.RESTAURANT_ADMIN) changes.reportWhatsapp = normalizeReportWhatsapp(body.reportWhatsapp); if (body.email !== undefined) { const email = body.email.trim().toLowerCase(); if (await this.users.exists({ email, _id: { $ne: user._id } })) throw new ConflictException('Já existe um usuário cadastrado com este e-mail.'); changes.email = email; } const updated = await this.users.findByIdAndUpdate(id, { $set: changes }, { new: true, runValidators: true }).select('name email phone reportWhatsapp role active deletedAt restaurantId createdAt updatedAt').populate('restaurantId', 'name tradeName').lean(); return userView(updated!); }
