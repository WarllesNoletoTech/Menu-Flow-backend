...
export class UsersController {
  ...
  @Patch(':userId/delete') async remove(@Param('userId') id: string, @Req() req: ActorRequest) { const user = await this.find(id); await this.protectAdmin(user, req.user.sub, 'Você não pode excluir sua própria conta.'); await this.users.updateOne({ _id: user._id }, { $set: { active: false, deletedAt: new Date() } }); await this.audit(req.user.sub, 'USER_DELETED', user._id); return { message: 'Usuário excluído; o histórico foi preservado.' }; }
